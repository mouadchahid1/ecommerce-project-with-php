            ...
            <div id="right">
                ...
                <div id="inner">
                    ...
                    <div id="in">
                        ...
                        <a style="margin-left: auto" id='sub-a'></a>
                        </div>
                    </div>
                    <div id="line"></div>
                    <div id="shipping">
                    ...
                    <div id="line"></div>
                       <div id="total">
                    <div id="in">
                        <a><strong >Total</strong></a>
                        <a style="margin-left: auto; font-weight:bold; font-size:20px" id='total-a'></a>
                        </div>
                    </div>
                    <div id="line"></div>
                    <div style="width: 90%; margin: 0 auto;display: flex;align-items: center;justify-content: center;margin-top: 20px">
...
function Do2(div_item, ind, h, ar){
    ...
    function LOADER(){
        ...
        function checkout(){
            if(local.checked){
            document.cookie='option=local.';    
                window.location.href='checkout.php'
            }
            else if(free.checked){
            document.cookie='option=free.';    
                window.location.href='checkout.php'
            }
            else {
                alert("Please select an shipping mode");
            }
        }
                
                function TOTAL(global){
                    document.getElementById('sub-a').innerHTML=global;
            ...
            function RemCart(count){
                ...
                function MaxIn(_in, index, pr){
                   var n = Number(document.getElementById("q"+_in).innerHTML) ;
                    document.getElementById("q"+_in).innerHTML= n+1;
                    Update(n+1, index);
                }
                function MinIn(_in, index, pr){
                    var n = Number(document.getElementById("q"+_in).innerHTML) ;
                   if(n>1) {document.getElementById("q"+_in).innerHTML= n-1;
                            Update(n-1, index);
                           }
                }
                function Update(n, index){
                    var product = localStorage.getItem("product"+index).substr(0, localStorage.getItem("product"+index).lastIndexOf(';'))+";"+n;
                    localStorage.setItem("product"+index, product);
                    RETRIEVE();
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    echo   "<div id='bar'>
            <div id='pic'>
            <img src='categories/".$cat_id."/".$id."/main.jpg'/>
            </div>
            <div id='name'>".$name."</div>
            <div id='price'>".$currency." ".$final_price."</div>
            <div id='quantity'>
            <button type='button' onclick=\"MinIn('".$in."','".$index."')\">-</button>
                <a id='q".$in."'>".$q2."</a>
                <button type='button' onclick=\"MaxIn('".$in."', '".$index."')\">+</button>
            </div>
            <div id='remove'>
            <button type='button' onclick=\"ItemIndex('".$index."')\">Remove</button>
                <button type='button' onclick=\"ItemIndex('".$index."')\" id='rem' style='width: 25px;height: 25px;'>x</button>
            </div>
            </div>";
